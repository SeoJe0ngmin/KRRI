...
- **상수 최소화**: 하드코딩은 물리 사실만. 직접·깨끗하게 관측되는 양(자이로 바이어스, 코너 잡음, 회전팔 A, 롤)은 **실시간 도출**;
  실시간으로 안 보이는 오차(정면 heading 치우침)는 표 대신 **회피 규칙 1개**(정면에선 heading 불신). 잡음 미분형 온라인 적응 금지.
  **config 에는 현장에서 사람이 정하는 값만**(차량·현장 바뀌면 바꿀 것). 필터 창·타임아웃 같은 구현 세부는 코드 내부 고정값(주석)으로 두고 config 에 올리지 않는다.
  v3 에서 v2 의 30프레임 안정성 판정 상수(STABLE_*, RELIABLE_TILT, MAX_HEADING_SIGMA, SIGMA_SAMPLES, CORNER_NOISE_PX 등)는 삭제 대상 — 상수 수는 순감이 목표.
...
**작업 정의 (2026-09-21 사용자 확정 — 이전의 "팔레트 포크 삽입" 가정은 오류였다)**:
지게차가 **탑재부(차고형 공간)에 들어가 주차**하는 것이 도킹이다. 팔레트에 포크를 꽂는 작업이 아니다 — 포크는 **접힌 채** 진입하고,
삽입 깊이(D)·포켓 폭·포크 폭 개념은 **해당 없음**. 탑재부 **폭이 지게차 폭과 거의 같아** 좌우 여유가 3 cm 뿐이고, 그래서 `LAT_TOL_M=0.030` 은
임시값이 아니라 **실물 근거값**이다. 탑재부 **길이(진입 깊이)는 미측정**(내일도 못 잼) → 2 m 로 보수 가정해 `HEAD_TOL_DEG` 를 **0.5°** 로 잡는다
(3 cm ÷ 2 m = 0.86°, 안전 여유 적용. 토론 2·3 의 "회전 σ_θ ≤ 0.5° 는 요건" 과 독립적으로 일치). 길이 실측 후 완화 가능.
**기준점 x_ref = 포크 끝**(카메라에서 `CAM_TO_REF_M = 1.52 m`) — 제일 앞이라 먼저 진입하고 비뚤면 먼저 닿는 부위.
**정지점 기하**: `DOCK_EXTRA_M=−2.02`(태그면 앞 2.02 m 정지)는 **실험용 벽 충돌 방지 값**. 태그는 ~3.3 m 에서 화면 밖 → 실험 블라인드 1.28 m.
실전 최종구간은 카메라-태그만으로 못 닫는다 → 대문제 4 항목(태그 높이/근거리 태그 2장/depth 정지/3.3 m 정렬 품질).
**미확정**: 탑재부 **길이(진입 깊이)** → HEAD_TOL_DEG 재계산 근거, 종방향 허용오차, byte2 비례 여부·97 의 m/s, 최소 제어량·데드존, σ_τ. 다음 실차 1순위 =
...
- 실측(2026-09-07): **회전 팔 A(카메라→회전중심)는 미확정** — config 의 1.46m 은 8m·자이로 사망 상태 1회 펄스 출처이고,
  18:14 로그 13회 회전의 Δβ/Δψ_IMU 0.77~0.95 는 A=+1.46 과 불일치(|A|≲0.4 또는 자이로 스케일 오차; 대문제 1 토론 결론, 실차 재측정 1순위).
  직진 정속 ~0.28~0.30m/s, 출발 지연 ~1s(주행)/~0.85s(회전, ROT_T0),
