...
HEAD_TOL_DEG = 2.0             # 각도 허용 오차 [도]. ★임시 느슨함 — 2도 잔차로 눈감는
                               # 2.43m 를 가면 85mm 밀려 LAT_TOL(30mm)과 안 맞는다. 정합값은
                               # asin(LAT_TOL/final길이)=0.7도지만, 회전 오버슈트(미실측)보다
                               # 조이면 회전 탁구가 된다. 오버슈트 실측 후 0.7~1.0 으로 조일 것
