...
동작 세 묶음
    Set1  forward 만큼 직진
    Set2  최소각 회전 -> lateral 만큼 이동 -> 반대로 90도 복귀 (그동안 태그 안 보임)
    Set3  태그가 안 보이면 반시계로 한 걸음씩, 보일 때까지

부품 (위에서 아래 순서)
    plan_lateral_clear / next_set3_step / drive_distance   기하·저수준 실행
    plan_step      두뇌. 측정값 하나 -> 동작 하나 (순수 계산)
...
용어
    ...
    heading   태그 축과 틀어진 각. **+가 반시계(왼쪽)**
    개루프    명령만 내리고 결과를 안 봄 — 직진이 이렇다 (N초 가라)
    폐루프    결과를 보며 조절 — 회전이 이렇다 (IMU 로 목표각에서 멈춘다)

CAN 명령은 다섯 개뿐이다 — 정지/전진/후진/좌회전/우회전. 세기 조절이 없어서
"얼마나"를 직진=시간, 회전=IMU 각도로 만든다.

IMU 계기판(GyroYaw)은 src/utils/imu_yaw.py. 여기서 만들지 않고 run.py 가
만들어 CanDriver 에 넣어 준다. 회전은 src/models/control/rot_control.py.

실측 기록과 남은 일은 ../NOTES.md.
"""
import asyncio
import copy
import math
...
# ── 부호 약속 ───────────────────────────────────────────────────────────────
# rotate_ccw -> heading 증가 (+ = 반시계 = 왼쪽), rotate_cw -> 감소.
# 카메라 쪽은 실측 확인(NOTES.md). 지게차 쪽이 반대로 돌면 rot_control.rotate_to()
# 안의 movement 매핑 두 곳만 맞바꾼다 — plan_step 의 ccw/cw 를 바꾸면 IMU 목표
# 부호까지 뒤집혀 wrong-way 정지만 반복된다.
...
def plan_step(m, state=None):
    ...
    # **흔들린다고 멈추지 않는다.** 실차는 항상 흔들리고, 멈춰서 다시 재도 같은
    # 값이 나와 도킹이 시작조차 안 된다. 30프레임 중앙값이라 흔들림에 강하고
    # 한 걸음이 짧아(<=1m) 다음 사이클에 고칠 수 있다. 판정은 계속 계산해서
    # 화면·기록에 남긴다(판단에만 안 씀). 실제 이탈은 전진 중 heading 감시와
    # 화면 잘림 감시가 따로 막는다.
    ...
    if abs(lateral_m) > LAT_TOL_M:
        if not m["reliable_angle"]:
            # **다가가는 것이 곧 해결이다.** 각도 잡음은 거리 제곱으로 줄어든다
            # (30cm 태그: 5m 1.49도 -> 3m 0.49도 -> 2m 0.27도). 못 믿는 각도로
            # Set2 를 하면 잡음을 쫓아 lateral 을 오히려 키운다. 여기서 멈춰 서면
            # 재도 값이 같아 영원히 hold 였다 — 시작 거리 전 구간이 그랬다.
    ...
    try:
        while True:
            ...
            if phase == "search":
                ...
                # 프레임마다 둘을 본다: heading(1도면 3m 가서 52mm 밀린다)과
                # 화면 여유(안 보면 한 명령에 잘리는 지점을 지나쳐 태그를 잃는다).
