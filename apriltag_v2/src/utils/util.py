...
def rpy2r(rpy_rad):
    ...
    Cpsi = np.cos(yaw)
    Spsi = np.sin(yaw)
    R = np.array([[Cpsi * Cthe, -Spsi * Cphi + Cpsi * Sthe * Sphi, Spsi * Sphi + Cpsi * Sthe * Cphi], [Spsi * Cthe, Cpsi * Cphi + Spsi * Sthe * Sphi, -Cpsi * Sphi + Spsi * Sthe * Cphi], [-Sthe, Cthe * Sphi, Cthe * Cphi]])
    assert R.shape == (3, 3)
    return R
...
def get_rotation_matrix_from_two_points(p_fr, p_to):
    ...
    if np.linalg.norm(p_to - p_fr) < 1e-08:
        return np.eye(3)
...
def soft_squash_multidim(x=np.random.randn(100, 5), x_min=-np.ones(5), x_max=np.ones(5), margin=0.1):
    x_squash = np.copy(x)
    dim = x.shape[1]
    for d_idx in range(dim):
        x_squash[:, d_idx] = soft_squash(x=x[:, d_idx], x_min=x_min[d_idx], x_max=x_max[d_idx], margin=margin)
    return x_squash
...
def is_point_feasible(point, obs_list):
    result = is_point_in_polygon(point, obs_list)
    if sum(result) == 0:
        return True
    else:
        return False
...
def get_interp_const_vel_traj(traj_anchor, vel=1.0, HZ=100, ord=np.inf):
    ...
    times_anchor = np.cumsum(dists / vel)
    L_interp = int(times_anchor[-1] * HZ)
    times_interp = np.linspace(0, times_anchor[-1], L_interp)
    traj_interp = np.zeros((L_interp, D))
...
def meters2xyz(depth_img, cam_matrix):
    ...
    xyz_img = np.stack([z_e, -x_e, -y_e], axis=-1)
    return xyz_img
