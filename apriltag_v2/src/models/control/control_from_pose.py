...
def _fmt_measure(m):
    ...
    return 'lat %+7.1fmm   fwd %6.3fm   head %+6.1f도   tilt %4.1f도   n %2d%s' % (m['lateral'] * 1000, m['forward'], m['heading_deg'], m['tilt_deg'], m['n'], flag)

def fwd_abort_deg(step_m):
    return math.degrees(math.asin(min(1.0, C.LAT_TOL_M / max(step_m, 1e-06))))
...
def plan_aim(lateral_m, forward_m, heading_deg, margin_px, st):
    ...
    dx = fwd_p - (S + A)
    bearing = math.degrees(math.atan2(-lat_p, dx))
    ...
    if dist_t <= C.AIM_AT_T_M or dx <= C.AIM_AT_T_M or tag_cut or (near_t and abs(lat_p) <= C.AIM_FINAL_MAX_LAT_M):
        if C.LAT_TOL_M < abs(lat_p) <= C.AIM_FINAL_MAX_LAT_M:
            ...
            if abs(turn) > C.AIM_FINAL_TOL_DEG:
                return ('rotate_ccw' if turn > 0 else 'rotate_cw', abs(turn), rot_sec_from_deg(turn), '정렬(태그 겨냥): 회전중심 lateral %.0fmm -> 목표 heading %+.1f도, 지금 %+.1f도 -> %+.1f도 회전' % (lat_p * 1000, h_star, heading_deg, turn))
            final_m = math.hypot(lat_p, fwd_p) - A + C.DOCK_EXTRA_M
            if final_m <= 0.0:
                return ('done', 0.0, 0.0, '도착. forward %.2fm' % forward_m)
            return ('final', final_m, fwd_sec_from_offset_piecewise(final_m), '태그 겨냥 끝(heading %+.1f도). 마지막 %.2fm. 그 뒤 정지' % (heading_deg, final_m))
        if abs(lat_p) > C.AIM_FINAL_MAX_LAT_M:
            if int(st.get('aim_backups', 0)) >= C.AIM_MAX_BACKUPS:
                return ('lost', 0.0, 0.0, 'T 에서 lateral %.0fmm 이 %d번 후진해도 안 맞는다. 수동전환' % (lat_p * 1000, C.AIM_MAX_BACKUPS))
            return ('aim_backup', C.AIM_BACKUP_M, fwd_sec_from_offset_piecewise(C.AIM_BACKUP_M), 'T 근처인데 회전중심 lateral %.0fmm — %.1fm 후진해 다시 조준' % (lat_p * 1000, C.AIM_BACKUP_M))
        if abs(heading_deg) > C.HEAD_TOL_DEG:
            return ('rotate_ccw' if heading_deg < 0 else 'rotate_cw', abs(heading_deg), rot_sec_from_deg(heading_deg), '정렬: T 에서 heading %+.1f도 를 지운다' % heading_deg)
        final_m = forward_m + C.DOCK_EXTRA_M
        if final_m <= 0.0:
            return ('done', 0.0, 0.0, '도착. forward %.2fm' % forward_m)
        return ('final', final_m, fwd_sec_from_offset_piecewise(final_m), '정렬 끝. 마지막 %.2fm (forward %.2fm + 여유 %.2fm). 그 뒤 정지' % (final_m, forward_m, C.DOCK_EXTRA_M))
    tag_dir = math.degrees(math.atan2(-lateral_m, forward_m))
    tag_off = abs(normalize_deg(tag_dir - bearing))
    if abs(bearing) > C.AIM_MAX_BEARING_DEG or tag_off > C.AIM_MAX_TAG_OFF_DEG:
        ...
        estimated_sec = rot_sec_from_deg(turn) + fwd_sec_from_offset_piecewise(distance_m) + rot_sec_from_deg(90.0)
        return ('sidestep', (turn, distance_m, direction), estimated_sec, '폴백 Set2 (T 방위각 %.0f도, 조준 뒤 태그 %.0f도 옆): lateral %.0fmm -> %+.1f도 회전 -> %.0fmm %s -> 90도 복귀' % (abs(bearing), tag_off, lateral_m * 1000, turn, distance_m * 1000, '전진' if direction == 'forward' else '후진'))
    turn = normalize_deg(bearing - heading_deg)
    if abs(turn) > C.AIM_TOL_DEG:
        return ('rotate_ccw' if turn > 0 else 'rotate_cw', abs(turn), rot_sec_from_deg(turn), '조준: T 방위각 %+.1f도, heading %+.1f도 -> %+.1f도 회전 (회전중심 T 까지 %.2fm, lateral %.0fmm)' % (bearing, heading_deg, turn, dist_t, lat_p * 1000))
    d = min(C.AIM_CHUNK_MAX_M, dist_t)
    b = math.radians(bearing)
    stop_forward = fwd_p - d * math.cos(b) - A * math.cos(b)
    return ('aim_drive', (d, bearing, stop_forward), fwd_sec_from_offset_piecewise(d), '조준 직진 %.2fm (회전중심 T 까지 %.2fm, 방위각 %+.1f도). 카메라 forward %.2fm 에서 조기 정지' % (d, dist_t, bearing, stop_forward))
...
def plan_step(m, state=None):
    ...
    if misses >= 1 and drove_forward and (not backed_up_once):
        return ('recover_backup', C.SEARCH_BACKUP_M, fwd_sec_from_offset_piecewise(C.SEARCH_BACKUP_M), '전진 중 태그를 놓쳤다. %.1fm 후진해서 다시 본다' % C.SEARCH_BACKUP_M)
    if misses >= C.SEARCH_AFTER_MISSES:
        ...
        if set3_rounds_done(rotations_done, half_fov) >= C.SEARCH_MAX_ROUNDS:
            return ('lost', 0.0, 0.0, '%d바퀴 찾아도 태그가 없다. 수동전환 — 사람이 확인해야 한다' % C.SEARCH_MAX_ROUNDS)
        turn = next_set3_step(half_fov, rotations_done)
        label = '연속 1바퀴 — 보이면 즉시 정지' if turn >= 360.0 else '반시계 %.0f도 걸음 (블러 없는 확인)' % turn
        return ('search', turn, rot_sec_from_deg(turn), 'Set3: %s (%d바퀴째)' % (label, set3_rounds_done(rotations_done, half_fov) + 1))
    if holds >= C.HOLD_MAX_CONSEC:
        return ('lost', 0.0, 0.0, '%d번 연속 멈춰 있다 (마지막 이유: %s). 수동전환 — 사람이 확인해야 한다' % (holds, st.get('hold_why') or '?'))
    if m is None:
        return ('hold', 0.0, 0.0, '태그를 못 봤다')
    lateral_m, forward_m, heading_deg = (m['lateral'], m['forward'], m['heading_deg'])
    if prev_forward is not None and st.get('last_action') in ('forward', 'final', 'aim_drive') and (forward_m > prev_forward + C.LAT_TOL_M):
        return ('hold', 0.0, 0.0, 'forward 가 늘었다 (%.2f -> %.2fm). 멈추고 다시 잰다' % (prev_forward, forward_m))
...
def _visible(res, tag_id):
    return res.docking.get(tag_id) is not None and res.quality.get(tag_id, {}).get('ok', True)
...
async def dock_live(pipe, driver, tag_id=None, max_steps=None, log=print, on_frame=None, n_frames=None, should_stop=None, record_dir=None):
    ...
    st = {'half_fov_deg': half_fov, 'prev_forward': None, 'drove_forward': False, 'backed_up_once': False}
    gen = iter(pipe)
    history, buf = ([], [])
    ...
    head_win = collections.deque(maxlen=int(C.AIM_ABORT_WINDOW))
    ...
    try:
        while True:
            ...
            if phase == 'search':
                ...
                if len(buf) >= n_frames or waited >= D.MEASURE_MAX_FRAMES:
                    ...
                    if saw_any:
                        ...
                        misses += 1
                    st['misses'] = misses
                    st['margin_px'] = sorted(margins)[len(margins) // 2] if margins else None
                    ...
                    record_event(record_dir, 'decision', step=step, action=action, why=why, misses=misses, margin_px=st.get('margin_px'), stable=(m or {}).get('stable'), reasons=(m or {}).get('reasons'), bearing_deg=st.get('aim_bearing'), dist_t=st.get('aim_dist_t'), lat_pivot=st.get('aim_lat_p'))
                ...
                if abort[0] is not None and task is not None and (not task.done()):
                    ...
                    if dev is not None and abs(dev) > abort[0]:
                        abort_hits += 1
                        if abort_hits >= 3:
                            why_cut = 'heading 중앙값 %+.2f도 (기준 %+.1f, 허용 %.2f도, %d프레임 창)' % (statistics.median(head_win), aim_heading, abort[0], head_win.maxlen)
                    ...
                    if d is not None and aim_stop is not None and (why_cut is None):
                        if d['forward'] <= aim_stop + C.AIM_STOP_LEAD_M:
                            ...
                            if stop_hits >= C.AIM_STOP_CONFIRM:
                                reached = True
                                why_cut = '카메라 forward %.2fm <= 목표 %.2fm + %.2f — 조기 정지' % (d['forward'], aim_stop, C.AIM_STOP_LEAD_M)
                        else:
                            stop_hits = 0
                    mg = _margin_px(res, tag_id)
            ...
            if on_frame is not None:
                ...
                on_frame(res, dict(info, phase=phase, step=step, left=left, n=len(buf), need=n_frames, tag=tag_id))
...
class DryRunDriver:
    ...
    async def rotate_by(self, deg):
        ...
        if self.realtime and sec:
            await asyncio.sleep(sec)
        return {'target': deg, 'turned': deg, 'overshoot': 0.0, 'ok': True, 'reason': 'dry-run'}
...
class CanDriver:
    ...
    def __init__(self, controller, yaw=None, log=None, record_dir=None):
        ...
        if yaw is not None and (not getattr(yaw, 'calibrated', False)) and log:
            log('       !! GyroYaw 가 보정 전이다 — 바이어스가 0 이라 5.5도/분 흘러간다. 정지 상태에서 calibrate() 를 부를 것')
    ...
    async def rotate_by(self, deg, timeout=None):
        return await rotate_to(self.c, self.yaw, deg, log=self.log, timeout=timeout, record=lambda result: record_event(self.record_dir, 'rotation', **result))

    async def stop(self):
        self.c.current_movement = 'stop'
__all__ = ['rot_sec_from_deg', 'rot_timeout_sec', 'fwd_abort_deg', 'drive_distance', 'plan_lateral_clear', 'plan_step', 'dock_live', 'DryRunDriver', 'CanDriver']
