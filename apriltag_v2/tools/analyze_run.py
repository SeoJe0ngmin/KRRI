...
from config.control import HEAD_TOL_DEG, LAT_TOL_M
from src.models.control.fwd_time_model import fwd_sec_from_offset_piecewise
...
def show_summary(events):
    ...
    if results:
        r = results[-1]
        label = {'done': '도착', 'manual': '수동전환 (태그를 끝내 못 찾음)', 'max_steps': '단계 초과 (수렴 실패)', 'user_stop': '사용자 중단', 'incomplete': '비정상 종료 (예외)'}.get(r['outcome'], r['outcome'])
    ...
    if measures:
        ...
        print('   마지막 측정: lat %+.0fmm %s   head %+.2f도 %s   fwd %.2fm' % (m['lateral'] * 1000, 'OK' if lat_ok else '**허용초과**', m['heading_deg'], 'OK' if head_ok else '**허용초과**', m['forward']))
    ...
    if decisions:
        print('   흔들림: %d/%d 사이클 (%.0f%%)' % (len(shaky), len(decisions), 100.0 * len(shaky) / len(decisions)))
        counts = {}
        for d in shaky:
...
def show_rotation(events):
    ...
    print('   오버슈트: 평균 %+.2f도  표준편차 %.2f도  최대 %+.2f도  (n=%d)' % (mean_over, sd_over, max(overs, key=abs), len(good)))
    print('   -> config/control.py 의 ROT_LEAD_DEG = %.1f 권장' % max(0.0, mean_over))
    fit = linear_fit([abs(r['target']) for r in good], [r['elapsed_sec'] for r in good]) if len(good) >= 3 else None
    if fit:
        t0, slope = fit
        if slope > 1e-06:
            print('   시간 적합: elapsed = %.2fs + 각도/%.1f도/s   (성공 %d회 기준)' % (t0, 1.0 / slope, len(good)))
            print('   -> rot_control.py 의 ROT_T0 = %.2f, ROT_DEG_PER_SEC = %.1f 권장' % (max(0.0, t0), 1.0 / slope))
    else:
...
def show_drive(events):
    ...
    for d in drives:
        ...
        actual = b['forward'] - a['forward']
        if commanded > 0.05:
            ratios.append(actual / commanded)
            print('   명령 %.2fm (%.1fs)  ->  실제 %.2fm   비율 %.3f' % (commanded, d['sec'], actual, actual / commanded))
    if ratios:
        mean_ratio = sum(ratios) / len(ratios)
        print('   -> 실제/명령 평균 %.3f.  1 에서 멀면 fwd_time_model.py 의 FWD_SCALE = %.3f 로 보정 (n=%d)' % (mean_ratio, 1.0 / mean_ratio, len(ratios)))
    else:
...
def show_timeline(events):
    ...
    for e in events:
        ...
        k = e['kind']
        if k == 'measure':
            line = 'lat %+7.1fmm  fwd %5.2fm  head %+6.2f도' % (e['lateral'] * 1000, e['forward'], e['heading_deg'])
        elif k == 'decision':
            line = '[%d] %s — %s' % (e.get('step', -1), e['action'], e['why'])
        elif k == 'rotation':
            line = '목표 %+.1f도 -> 실제 %s (%.1fs) %s' % (e['target'], '%+.1f도' % e['turned'] if e.get('turned') is not None else '?', e.get('elapsed_sec', 0.0), '' if e.get('ok') else '!! ' + e.get('reason', ''))
        elif k == 'drive':
