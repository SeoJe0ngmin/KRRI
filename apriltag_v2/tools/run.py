...
from config.main import TAG_ID, TAG_SIZE_M
from src.models import TagPipeline
from src.models.control.control_from_pose import CanDriver, DryRunDriver, dock_live
from config.main import MAX_STEPS
from src.models.detection.image import intrinsics_from_ref
from src.utils.imu_yaw import GyroYaw
from src.utils.event_log import snapshot_config
PHASE_KO = {'measure': '측정 중', 'command': '명령 실행 중', 'search': 'Set3: 태그 찾는 중', 'final': '마지막 접근 중', 'done': '끝', 'manual': '!! 수동전환 필요 !!'}
from src.utils.imu_yaw import imu_panel_lines as imu_lines
...
def make_view(args, yaw=None):
    ...
    def on_frame(res, info):
        ...
        ctx = {'source': 'run', 'w': vis.shape[1], 'h': vis.shape[0], 'frame': res.index, 'fps': 0.0, 'intr': res.intrinsics, 'intr_origin': 'factory', 'intr_assumed': False, 'tag_size': args.tag_size, 'size_assumed': False, 'res': res, 'primary': det, 'paused': False, 'draw': 'cube', 'family': 'tag36h11', 'method': 'auto', 'quad_blur': 0.0, 'stats': None}
...
def _check_can_templates():
    ...
    for name in ('stop', 'forward', 'backward', 'rotate_ccw', 'rotate_cw'):
        ...
        if bad:
            raise SystemExit('!! CAN 템플릿 %s 의 byte%s 가 중립(%d)이 아니다 — byte4 는 이 지게차에서 포크 리프트다. 출발하지 않는다' % (name, bad, AN_NEUTRAL))
    print('  CAN 템플릿 확인: rotate_ccw byte1=%d  rotate_cw byte1=%d  forward byte2=%d  backward byte2=%d  (그 외 바이트 전부 중립)' % (M['rotate_ccw'][1], M['rotate_cw'][1], M['forward'][2], M['backward'][2]))
...
async def main_async(args):
    ...
    if not args.no_imu:
        try:
            ...
            print('     회전축 %s,  잡음 %.3f 도/s,  드리프트 %.2f 도/분' % (rep['axis_src'], rep['noise_dps'], rep['drift_dpm']))
            if rep['moving']:
    ...
    try:
        ...
        if args.record_events:
            ...
            _cfg = snapshot_config(record_dir)
            if _cfg:
                print('  config 스냅샷 -> %s' % _cfg)
            if not args.dry_run:
                driver.record_dir = record_dir
            else:
                print('  (dry-run: measure/decision/result 만 기록. rotation/drive 는 실주행에서만)')
            print('  운행 기록 -> %s/' % record_dir)
        print('  시작\n')
        await dock_live(pipe, driver, tag_id=args.tag_id, max_steps=args.max_steps, on_frame=on_frame, record_dir=record_dir)
...
def main():
    ...
    ap.add_argument('--dry-run', action='store_true', help='CAN 없이 순서만 본다. 카메라는 실제로 쓴다')
    ap.add_argument('--show', action='store_true', help='주행하면서 화면도 띄운다 (같은 프로세스라 카메라 충돌 없음)')
    ap.add_argument('--tag-id', type=int, default=TAG_ID)
    ap.add_argument('--tag-size', type=float, default=TAG_SIZE_M, help='태그 한 변 [m], 검은 테두리 바깥까지')
    ap.add_argument('--max-steps', type=int, default=MAX_STEPS)
    ap.add_argument('--no-imu', action='store_true', help='자이로를 안 연다. 회전이 미측정 시간모델 개루프가 된다')
    ap.add_argument('--record-events', action='store_true', help='운행 기록을 %s/시각/ 폴더에 남긴다 (실행마다 새 폴더, 종류별 .jsonl, ts 로 병합 가능). 분석은 tools/analyze_run.py. dry-run 에선 카메라 쪽(measure/decision/result)만 남는다' % EVENT_LOG_DIR)
