...
def step_import():
    ...
    try:
        ...
        if sys.version_info < (3, 10):
            info('→ ★ 지금 파이썬은 %d.%d 다. PyPI 리눅스 x86-64 휠이 cp310 부터라 3.9 이하 env 는 pip 설치가 아예 안 된다.' % sys.version_info[:2])
            info('   3.11 env 를 쓰거나(예: conda activate krri) 소스 빌드해야 한다.')
        return None
    ver = getattr(rs, '__version__', None) or getattr(rs, '__full_version__', None) or '(버전 정보 없음)'
...
def step_gui(enabled=True):
    ...
    try:
        ...
        info('→ opencv-python-headless 가 깔려 있으면 imshow 자체가 없다. pip install opencv-python 으로 교체.')
        try:
...
def step_frame_metadata(rs):
    ...
    try:
        ...
        if have == want:
            mark('PASS', '프레임별 노출/게인', '%d 항목 - 실패 프레임 로그를 남길 수 있다' % len(got))
        elif got:
            mark('WARN', '프레임별 노출/게인', '%d 항목만 온다 (없는 것: %s)' % (len(got), ', '.join(sorted(want - have)) or '-'))
        else:
        ...
        try:
            ...
            if csen is not None and mv is not None and f.supports_frame_metadata(mv) and csen.supports(rs.option.exposure):
                md = int(f.get_frame_metadata(mv))
                opt = float(csen.get_option(rs.option.exposure))
                if abs(md - opt) <= max(1.0, opt * 0.02):
                    mark('PASS', '노출 눈금 대조', 'metadata=%d, option=%.0f 일치 -> 컬러는 100us 눈금(=%.1fms)' % (md, opt, md * 100.0 / 1000.0))
                else:
                    mark('WARN', '노출 눈금 대조', 'metadata=%d vs option=%.0f 불일치 - 단위 가정을 다시 봐라' % (md, opt))
...
def step_no_device_help():
    ...
    info('/dev/bus/usb : %s' % ('있음' if Path('/dev/bus/usb').exists() else '없음 → USB 가 WSL 로 전달된 적 없음'))
...
def main():
    ap = argparse.ArgumentParser(description='RealSense 진단 - SDK CLI 가 못 하는 것만 (절대 죽지 않는다)')
    ap.add_argument('--no-gui', dest='gui', action='store_false', help='GUI 검사 생략')
    ap.add_argument('--force-mac', action='store_true', help='macOS 에서도 SDK 로 장치를 열어 본다 (2.56.5 는 여기서 segfault 난다)')
    args = ap.parse_args()
    print('=' * 74)
    ...
    print('\n' + '=' * 74)
    print(' 결과: PASS %d  FAIL %d  WARN %d  SKIP %d' % (TALLY['PASS'], TALLY['FAIL'], TALLY['WARN'], TALLY['SKIP']))
    if ndev == 0:
